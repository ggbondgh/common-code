...
{
    T1CR = DEF_SET_BIT0 | DEF_SET_BIT1;//定时模式,CPU,8分频
    T1CNT = 100 - 1;                   //100us
    T1LOAD = 100 - 1;
    T1EN = 1;
    T1IE = 1;
...
void HIRC_User(void)
{
    if (FLAG_PFRCR_ADD_DEC)
    {
        ...
        {
            g_hirc_temp ++;
        }else FLAG_PFRCR_ADD_DEC = 0;  //加到PFRCR_back   开始递减
    }
    else
    {
        if(g_hirc_temp > 0)
        {
            g_hirc_temp --;
        }else FLAG_PFRCR_ADD_DEC = 1;  //减到0  开始递增
    }
    HIRCCAL = g_hirc_temp;             //修改微调寄存器
}
/************************************************
;  *    @函数名            : interrupt
;  *    @说明              : 中断函数
