...
#define  PWM_Duty_Min  25   // 8%
#define  PWM_Duty_Half 127  // 50%

// 不跳频  4.8M
#define	   	TK_SENS_LEVEL_H1  0x0A     	   	//;TK0触摸灵敏度级别1
#define	   	TK_SENS_LEVEL_H2  0x0C     	   	//;TK0触摸灵敏度级别2
#define	   	TK_SENS_LEVEL_H3  0x0E     	   	//;TK0触摸灵敏度级别3
#define	   	TK_SENS_LEVEL_H4  0x12     	   	//;TK0触摸灵敏度级别3
#define	   	TK_SENS_LEVEL_H5  0x17    	   	//;TK0触摸灵敏度级别3
#define	   	TK_SENS_LEVEL_L   0xFF     	   	//;TK0触摸灵敏度级别


#define FilterCnt_Value    	8
#define Const_SEN_T_Key0   	50
...
#define    	T0_13ms_Flag        flag1.bits.bit5	   	 // 标志位
#define    	T_Long_press_Flag   flag1.bits.bit6	   	 // 标志位
#define    	PWM_Flag_key        flag1.bits.bit7	   	 // 标志位

//#define    	LED_Flag       	     flag2.bits.bit0    // 标志位
#define    	PWM_Update_F      	    flag2.bits.bit1    // 标志位
#define    	TK0_Key_Release_Flag    flag2.bits.bit2    // 标志位
#define     BAT_Low_V_Flag         	flag2.bits.bit3    // 标志位
//#define    	Duty_Memory_Flag        flag2.bits.bit4    // 标志位
//#define    	PWM_Double_Flag    	   	flag2.bits.bit5    // 标志位
//#define    	PWM_3_Gear_Flag    	   	flag2.bits.bit6    // 标志位
//#define    	PWM_HML_Flag       	   	   	flag2.bits.bit7    // 标志位
...
uint16_t u16_TK_Reset_Cnt; 
uint8_t u8_WDT_18ms_Cnt;
...
void Uart_Send_Data(void) // 19200  P14
{
       ...
       //  	uart_send_c = u16_temp_val & 0x00FF;
       //  	Uart_Send_One_Byte();
       //  	uart_send_c =  (u16_temp_val & 0xff00) >> 8; 
       //  	Uart_Send_One_Byte();
}
#endif
...
{
    T1CR = 0x8A;    // 100_01_001 使能T1、时钟为FOSC、4分频
    T1CNT = 255 - 1;
    T1LOAD = 255 - 1;  	//16k
...
void TK_Sens_Threshold_value_Detection(void)
{
 ...
 //    	P15D = 0;
    ...
    if(u16_temp_val > 400) // 100nF    	   	高灵敏度
    {
   	   	T2LOADH = TK_SENS_LEVEL_H5;
    }
   	else if(u16_temp_val > 80) // 22或27nF   中灵敏度
    {
   	   	T2LOADH = TK_SENS_LEVEL_H4;
    } 
   	else if(u16_temp_val > 15) // 4.7nF   中灵敏度
    {
   	   	T2LOADH = TK_SENS_LEVEL_H3;
    } 
   	else if(u16_temp_val > 2) // 1nF   中灵敏度
    {
   	   	T2LOADH = TK_SENS_LEVEL_H2;
    } 
   	else //if(u16_temp_val > 10) // 悬空      低灵敏度
    {
...
void PWM_Direction_Judgment(void)
{
   	...
   	else if (T_key_Value == 1)
   	{
   	   	if(PWM_Duty_Y > PWM_Duty_Half)
   	   	{
   	   	   	PWM_Flag_key = 1;  //调亮调暗标志位状态变换
   	   	}
   	   	else
   	...
   	else if(T_key_Value == 3)
   	{
   	   	if(PWM_Duty_WY > PWM_Duty_Half)
   	   	{
            PWM_Flag_key = 1;  //调亮调暗标志位状态变换
   	   	}
   	   	else
...
void T_Key_value_Read(void)
{
   	...
   	else if(!T_Key0_Signal)
   	{
   	   	...
   	   	if(TK0_Key_Cnt > 0 && TK0_Key_Cnt < 42)
   	   	{
   	   	   	PWM_Update_F = 1;

       	   	T_key_Value ++;    	//按键值累加    	   	   	
            if(T_key_Value > 3 ) // 1 2 3 按键值循环（三色调光）
            {
                T_key_Value = 0;
            }
...
void LED_PWM_Control(void)
{
   	...
   	if(T_Long_press_Flag) //长按无极调光（255极）
   	{
   	   	...
   	   	if(PWM_Flag_key == 0)  	//逐渐变暗
   	   	{
   	   	   	if(T_key_Value == 2)
   	   	   	{
   	   	   	   	if(PWM_Duty_W < PWM_Duty_Max) //最大亮度 100%
   	   	   	   	{
   	   	   	   	   	PWM_Duty_W ++;
   	   	   	   	}
   	   	   	}
   	   	   	...
   	   	   	{
   	   	   	   	if(PWM_Duty_Y < PWM_Duty_Max) //最大亮度 100%
   	   	   	   	{
   	   	   	   	   	PWM_Duty_Y ++;
   	   	   	   	}
   	   	   	}  	
            ...
            {
                if (PWM_Duty_WY < PWM_Duty_Max) //最大亮度 100%
                {
                    PWM_Duty_WY ++;
                }
            }
   	   	...
   	   	{  	
   	   	   	if(T_key_Value == 2)
   	   	   	{
   	   	   	   	if(PWM_Duty_W > PWM_Duty_Min ) //最小亮度PWM值：255 * 8%，
   	   	   	   	{
   	   	   	   	   	PWM_Duty_W --; 
   	   	   	   	}
   	   	   	}
   	   	   	...
   	   	   	{
   	   	   	   	if(PWM_Duty_Y > PWM_Duty_Min ) //最小亮度PWM值：255 * 5%，
   	   	   	   	{
   	   	   	   	   	PWM_Duty_Y --; 
   	   	   	   	}
   	   	   	}
            ...
            {
                if (PWM_Duty_WY > PWM_Duty_Min) //最小亮度PWM值：255 * 5%，
                {
                    PWM_Duty_WY--;
                }
            }
...
void Power_Low_V_OFF(void)
{
   	if(T_key_Value && BAT_Low_V_Flag)
    {
   	   	Power_Low_V_OFF_Cnt ++;

   	   	if(Power_Low_V_OFF_Cnt > 200) // 2.6S
        {
...
void TK_5S_Reset(void)
{
    if(T_Key0_Signal)
    {
       	 u16_TK_Reset_Cnt ++;
       	 if(u16_TK_Reset_Cnt >= 385) // 13ms 5S
        {
       	   	  //u8_TK_Reset_Cnt = 0;
       	   	  TK0_release_();
        }
...
{      	                 
   	   	...
   	   	Delay_us_2(0x8001); // 4M 约100ms

   	   	TK_Sens_Threshold_value_Detection(); // 5档灵敏度
       	...
       	PWM_Duty_W = PWM_Duty_Max;
       	PWM_Duty_Y = PWM_Duty_Max;
       	PWM_Duty_WY = PWM_Duty_Max;

   	   	T_key_Value = 0; // Power_ONOFF_Flag
