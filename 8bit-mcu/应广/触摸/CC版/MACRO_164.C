...
Touch_X  MACRO TKX,TK_Math,TK_Mode  
	...
	.ifnb <TK_Mode>
		...
		.if		TK_Mode==0
				tps2 |=0b00_00_0000;//CS-VCC
		.elseif 	TK_Mode==1
				tps2 |=0b01_00_0000;//CS-GND
		.else
	...
	.endif
	// touch_key_init();
	TS = 0x23; //IHRC/4, 0.4*VCC, 128*CLK
	$ TCC TK_RUN;
	do
	{
		TOUCH_CH = TCC & 0x70; //获取状态
		TK_Math		=	0xFFFF;
	}while (TOUCH_CH); //TK_STOP时退出
	TK_Math $ 1 = TKCH & 0x0F;
	TK_Math $ 0 = TKCL;
