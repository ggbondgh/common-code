...
//
//	Touch_Init()		初始化函数
//	Touch_Func()		调用函数，信号为Tk_Signal1~11
...
word TKEnvironment[Touch_sum] = { 0 }; //环境值
word TKProcessEvironment[Touch_sum] = { 0 }; //跟随值
word u16_TK_val = 0;   //用于一阶滤波的临时变量
word u16_temp_val = 0; //临时变量
...
void Touch_Get_val() //获取环境值
{
	...
	u16_TK_val += TK_Math;
	TK0Environment = (u16_TK_val >> 2); //一阶滤波
#endif
#if Touch_sum >= 2
	Touch_X Touch_2, TK_Math, 0
...
{
	.forc CC #Count
		if (!Pre_T_Key_Release#CC) //按下检测
		{
			if (TK#CC#Environment < TK#CC#ProcessEvironment) //环境值<跟随值，向下跟随
			{
				Env_Fix_Cnt#CC++;
				if (Env_Fix_Cnt#CC >= Const_Env_Fix_Cnt)
				{
					Env_Fix_Cnt#CC = 0;
					TK#CC#ProcessEvironment--; //跟随值自减
				}
	
			else //到达环境值后
			{
				Env_Fix_Cnt#CC = 0;
				TK#CC#ProcessEvironment = TK#CC#Environment; //一直跟随环境值
			}
			u16_temp_val = TK#CC#Environment + Const_SEN_T_Key#CC; //环境值+灵敏度阈值
...
*/	
				...
				if (Fliter_cnt#CC >= Const_Fliter_cnt)
				{
					...
					/*	#if TO#cc#_ACTIVE
								...
								#endif
						#endif*/
					TK_Release_Value#CC = TK#CC#ProcessEvironment - Const_SEN_T_Key#CC; //赋予松手值
					//					LongKeyRestCount=0;
					//					TK1_res_cnt = 0;
					Pre_T_Key_Release#CC = 1; //进入松手检测
				}
			}
			else //未达到触摸
			{
		...
		{
			if (TK#CC#Environment > TK#CC#ProcessEvironment) //环境值>跟随值，向上跟随
			{
				Env_Fix_Cnt#CC++;
				if (Env_Fix_Cnt#CC >= Const_Env_Fix_Cnt)
				{
