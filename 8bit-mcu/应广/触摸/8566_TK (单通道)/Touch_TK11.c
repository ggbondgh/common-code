...
使用方法：
	...
	2.在主循环-定时5ms-中调用以下三个函数（除了TK11_Scan外其余两个多通道共用）：
...
----------------------------------------------------------------------------------------------
注意：
	1.例程用的是Key11，如果用其他触摸脚需要把“11”全部改成相应数字,并在TK11_Scan()中使能用到的TKE通道位
*****************************************************************************************************/
...
word u16_get_val_TK11;		 //读取值
word u16_Press_val_TK11;     //按下后的读取值
word u16_release_val_TK11;	 //松开后的读取值

eword u16_TK11_environment_val_Sum = 0;  //用于暂时存取累加值
word u16_TK11_environment_val;			 //环境值（用于跟随读取值）
word u16_release_val2_TK11 = 0;  		 //按下后的 “环境值-按下阈值”
bit  TK11_Release_Flag;

word u16_temp_val_TK11 = 0;   //一阶滤波用到的临时变量
...
void	TK_Release_TK11(void)
{
	...
	u16_TK11_environment_val = u16_get_val_TK11;
	TK11_Filter_Cnt = 0;
...
{	
	...
	TKE2 = 0b_0000_0010;	//TKE2:bit0~4 => TK8~12  使能TK11
	...
	u16_get_val_TK11 =  (TKCH << 8) | TKCL;

	//一阶滤波
	u16_temp_val_TK11 = u16_get_val_Temp_TK11 << 2;
	u16_temp_val_TK11 -= u16_get_val_Temp_TK11;
	u16_temp_val_TK11 += u16_get_val_TK11;
	u16_get_val_Temp_TK11 = u16_temp_val_TK11 >> 2;

	u16_get_val_TK11 = u16_get_val_Temp_TK11;


	if(Pre_T_Key11_Release) //检测是否按下
	{
		u16_Press_val_TK11 = u16_get_val_TK11 + Const_SEN_T_Key11;
		
		if(u16_Press_val_TK11 < u16_TK11_environment_val)	 //按下，跟随值>读取值+按下阈值
		{
		//	Const_Env_Dw_Fix_Speed = 5;
			...
			if(FilterCnt_TK11 >= FilterCnt_Value)  //滤波，检测到连续多次才视为按下
			{
				...
				u16_release_val2_TK11 = u16_TK11_environment_val - Const_SEN_T_Key11; //留取，给后面松开检测用
			}
		}
		else //更新环境值，跟随读取值
		{
			...
			// 检测多次读取值取平均值
			TK11_Filter_Cnt ++;
			u16_TK11_environment_val_Sum += u16_get_val_TK11; //读取值累加
			...
			{
				if(u16_get_val_TK11 > u16_TK11_environment_val)
				{
					u16_TK11_environment_val = u16_get_val_TK11;
				}
			}
	...
	else   //检测是否松开
	{
		...
		if(u16_get_val_TK11 > u16_TK11_environment_val)
		{
			u16_release_val_TK11 = u16_get_val_TK11 - u16_TK11_environment_val;
		}
		else
		{
			u16_release_val_TK11 = 0;
		}

		if(u16_get_val_TK11 > u16_release_val2_TK11 || u16_release_val_TK11 > Const_SEN_T_Key11_Release)	//松开
		{
			TK11_Release_Cnt ++;
